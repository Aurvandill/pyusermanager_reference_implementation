...
def get_default_response():
    default_response = HTTPResponse(status=HTTPStatus.IM_A_TEAPOT,body=json.dumps({"error":"this should never happen lol"}))
    default_response.headers['Access-Control-Allow-Origin'] = '*'
    default_response.headers['Access-Control-Allow-Methods'] = 'PUT, GET, POST, DELETE, OPTIONS'
    default_response.headers['Access-Control-Allow-Headers'] = 'Origin, Accept, Content-Type, X-Requested-With, X-CSRF-Token'
    default_response.body = get_json_from_args(Alert("oh god something went terribly wrong",ALERT_TYPE.DANGER))
...
def api_verify_token(username):
    ...
    try:
        ...
        return_response.body = get_json_from_args(Alert("No Token supplied!",ALERT_TYPE.DANGER),Redirect("/"))
    ...
    try:
        ...
        return_response.status=HTTPStatus.BAD_REQUEST
        return_response.body = get_json_from_args(Alert("Token Type is not valid!",ALERT_TYPE.DANGER),Redirect("/"))
        return return_response
...
def login():
    ...
    try:
        ...
        return_response.status = HTTPStatus.BAD_REQUEST
        return_response.body = get_json_from_args(Alert("required vars missing",ALERT_TYPE.WARNING))
        return return_response
    ...
    try:
        ...
        return_response.status = HTTPStatus.BAD_REQUEST
        return_response.body = get_json_from_args(Alert("User does not exist!",ALERT_TYPE.WARNING))
        return return_response
    ...
    if success:
        ...
        try:
            ...
            return_response.body = get_json_from_args({"Login":{"valid_day":valid_days,"token":token}},Alert("Successfull Login",ALERT_TYPE.INFO),Redirect(f"/user/{user}"))
        except Exception:
            return_response.status = HTTPStatus.INTERNAL_SERVER_ERROR
            return_response.body = get_json_from_args(Alert("could not generate token!",ALERT_TYPE.DANGER))
    else:
        return_response.status = HTTPStatus.UNAUTHORIZED
        return_response.body = get_json_from_args(Alert("password/User Combination is wrong!",ALERT_TYPE.WARNING))
    ...
    if token is not None:
        ...
        if success:
            ...
            if LoginConfig.admin_group_name in perms:
                header = filestuff.get_template("header_logged_in_admin.html",**template_vars)
            else:
                header = filestuff.get_template("header_logged_in.html",**template_vars) 
...
def api_logout_user():
    ...
    if token is not None:
        try:
            ...
            return_response.body = get_json_from_args(logout,Alert("successfull Logout",ALERT_TYPE.INFO),Redirect("/"))
        except TokenMissingException:
            return_response.status=HTTPStatus.BAD_REQUEST
            return_response.body = get_json_from_args(logout,Alert("Token does not exist!",ALERT_TYPE.DANGER))
        except ValueError:
            return_response.status=HTTPStatus.BAD_REQUEST
            return_response.body = get_json_from_args(logout,Alert("cant Log Out User from Different IP!",ALERT_TYPE.WARNING))
    else:
        return_response.status=HTTPStatus.BAD_REQUEST
        return_response.body = get_json_from_args(Alert("no Token transmitted",ALERT_TYPE.DANGER))
